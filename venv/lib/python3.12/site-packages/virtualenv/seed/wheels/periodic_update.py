...
def add_wheel_to_update_log(
    wheel: Wheel, for_py_version: str, app_data: AppData
) -> None:
...
class NewVersion:  # noqa: PLW1641
    ...
    def from_dict(cls, dictionary: dict[str, str | None]) -> NewVersion:
        return cls(
            ...
            found_date=load_datetime(
                dictionary["found_date"]
    ...
    def use(
        self,
        now: datetime,
        ignore_grace_period_minor: bool = False,
        ignore_grace_period_ci: bool = False,
    ) -> bool:  # noqa: FBT002
...
class UpdateLog:
    ...
    @classmethod
    def from_app_data(
        cls, app_data: AppData, distribution: str, for_py_version: str
    ) -> UpdateLog:
...
def release_date_for_wheel_path(dest: Path) -> datetime | None:
    ...
    if content is not None:
        try:
            upload_time = content["releases"][wheel.version][0][
                "upload_time"
...
def _run_manual_upgrade(
    app_data: AppData, distribution: str, for_py_version: str, env: dict[str, str]
) -> None:
