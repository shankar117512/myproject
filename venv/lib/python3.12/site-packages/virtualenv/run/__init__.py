...
def session_via_cli(
    ...
    options.py_version = (
        parser._interpreter.version_info
    )  # noqa: SLF001  # ty: ignore[invalid-assignment, unresolved-attribute]
    creator, seeder, activators = tuple(
        e.create(options) for e in elements  # ty: ignore[invalid-argument-type]
    )  # create types
    return Session(
...
def load_app_data(
    ...
    options: VirtualEnvOptions | None,
) -> VirtualEnvOptions:
    parser.add_argument(
        "--read-only-app-data",
        action="store_true",
...
def _do_report_setup(
    parser: VirtualEnvConfigParser, args: list[str] | None, setup_logging: bool
) -> None:
    level_map = ", ".join(
