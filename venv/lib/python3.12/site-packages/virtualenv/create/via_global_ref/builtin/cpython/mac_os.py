...
class CPythonmacOsFramework(CPython, ABC):
    ...
    ) -> Generator[tuple[Path, list[str], str, str]]:
...
class CPython3macOsFramework(CPythonmacOsFramework, CPython3, CPythonPosix):
    ...
    def sources(
        cls, interpreter: PythonInfo
    ) -> Generator[PathRef]:  # ty: ignore[invalid-method-override]
...
def _builtin_change_mach_o(
    maxint: int,
) -> Callable[[str, str, str], None]:  # noqa: C901
    ...
    class FileView:
        ...
        def __init__(
            self,
            file_obj: FileView | BufferedRandom,
            start: int = 0,
            size: int = maxint,
        ) -> None:
    ...
    def mach_o_change(at_path: str, what: str, value: str) -> None:  # noqa: C901
        ...
        def do_macho(file: FileView, bits: int, endian: str) -> None:
            ...
            for _ in range(n_commands):
                ...
                if cmd == LC_LOAD_DYLIB:
                    ...
                    file.seek(
                        where + name_offset, os.SEEK_SET
                    )  # ty: ignore[unsupported-operator]
                    # Read the NUL terminated string
                    load = file.read(
                        cmd_size - name_offset
                    ).decode()  # ty: ignore[unsupported-operator]
        ...
        def do_file(
            file: FileView | BufferedRandom, offset: int = 0, size: int = maxint
        ) -> None:
...
class CPython3macOsBrew(CPython3, CPythonPosix):
    ...
    def setup_meta(
        cls, interpreter: PythonInfo
    ) -> BuiltinViaGlobalRefMeta:  # noqa: ARG003
