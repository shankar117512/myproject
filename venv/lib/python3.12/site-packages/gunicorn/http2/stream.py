...
class HTTP2Stream:
    ...
    def receive_headers(self, headers, end_stream=False):
        ...
        if self.state == StreamState.IDLE:
            ...
            raise HTTP2StreamError(
                self.stream_id, f"Cannot receive headers in state {self.state.name}"
            )
    ...
    def receive_data(self, data, end_stream=False):
        ...
        if not self.can_receive:
            raise HTTP2StreamError(
                self.stream_id, f"Cannot receive data in state {self.state.name}"
            )
    ...
    def receive_trailers(self, trailers):
        ...
        if not self.can_receive:
            raise HTTP2StreamError(
                self.stream_id, f"Cannot receive trailers in state {self.state.name}"
            )
    ...
    def send_headers(self, headers, end_stream=False):
        ...
        if not self.can_send:
            raise HTTP2StreamError(
                self.stream_id, f"Cannot send headers in state {self.state.name}"
            )
    ...
    def send_data(self, data, end_stream=False):
        ...
        if not self.can_send:
            raise HTTP2StreamError(
                self.stream_id, f"Cannot send data in state {self.state.name}"
            )
    ...
    def send_trailers(self, trailers):
        ...
        if not self.can_send:
            raise HTTP2StreamError(
                self.stream_id, f"Cannot send trailers in state {self.state.name}"
            )
    ...
    def _half_close_local(self):
        ...
        if self.state == StreamState.OPEN:
            ...
            raise HTTP2StreamError(
                self.stream_id, f"Cannot half-close local in state {self.state.name}"
            )
    ...
    def _half_close_remote(self):
        ...
        if self.state == StreamState.OPEN:
            ...
            raise HTTP2StreamError(
                self.stream_id, f"Cannot half-close remote in state {self.state.name}"
            )
