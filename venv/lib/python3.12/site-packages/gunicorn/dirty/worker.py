...
class DirtyWorker:
    ...
    SIGNALS = [
        getattr(signal, "SIG%s" % x) for x in "ABRT HUP QUIT INT TERM USR1".split()
    ]
        ...
        try:
            ...
            for path, app in self.apps.items():
                ...
                try:
                    ...
                    self.log.error("Failed to initialize dirty app %s: %s", path, e)
                    raise
        except Exception as e:
    ...
    def run(self):
        ...
        self._executor = ThreadPoolExecutor(
            max_workers=num_threads, thread_name_prefix=f"dirty-worker-{self.pid}-"
        )
    ...
    async def _run_async(self):
        ...
        self.log.info("Dirty worker %s listening on %s", self.pid, self.socket_path)
    ...
    async def handle_request(self, message, writer):
        ...
        if msg_type != DirtyProtocol.MSG_TYPE_REQUEST:
            response = make_error_response(
                request_id, DirtyWorkerError(f"Unknown message type: {msg_type}")
            )
        ...
        try:
            ...
            self.log.error("Error executing %s.%s: %s\n%s", app_path, action, e, tb)
            response = make_error_response(
                request_id,
                DirtyAppError(str(e), app_path=app_path, action=action, traceback=tb),
            )
            await DirtyProtocol.write_message_async(writer, response)
    ...
    async def _stream_sync_generator(self, request_id, gen, writer):
        ...
        try:
            ...
            response = make_error_response(
                request_id, DirtyAppError(str(e), traceback=tb)
            )
            await DirtyProtocol.write_message_async(writer, response)
        finally:
            gen.close()
        ...
        try:
            result = await asyncio.wait_for(
                loop.run_in_executor(
                    self._executor, lambda: app(action, *args, **kwargs)
                ),
                timeout=timeout,
            )
            return result
            ...
            self.log.warning(
                "Execution timeout for %s.%s after %ds", app_path, action, timeout
            )
            raise DirtyTimeoutError(
                f"Execution of {app_path}.{action} timed out", timeout=timeout
            )
