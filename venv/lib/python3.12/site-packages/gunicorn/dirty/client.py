    ...
    def connect(self):
        ...
        try:
            ...
            raise DirtyConnectionError(
                f"Failed to connect to dirty arbiter: {e}", socket_path=self.socket_path
            ) from e
    ...
    def _execute_locked(self, app_path, action, args, kwargs):
        ...
        try:
            ...
            raise DirtyTimeoutError(
                "Timeout waiting for dirty app response", timeout=self.timeout
            )
    ...
    async def connect_async(self):
        ...
        try:
            self._reader, self._writer = await asyncio.wait_for(
                asyncio.open_unix_connection(self.socket_path), timeout=self.timeout
            )
        except asyncio.TimeoutError:
            raise DirtyTimeoutError(
                "Timeout connecting to dirty arbiter", timeout=self.timeout
            )
    ...
    async def execute_async(self, app_path, action, *args, **kwargs):
        ...
        try:
            ...
            response = await asyncio.wait_for(
                DirtyProtocol.read_message_async(self._reader), timeout=self.timeout
            )
...
class DirtyStreamIterator:
    ...
    def __init__(self, client, app_path, action, args, kwargs, idle_timeout=None):
        self.client = client
    ...
    def _read_next_chunk(self):
        ...
        with self.client._lock:
            ...
            if now >= self._deadline:
                ...
                raise DirtyTimeoutError(
                    "Stream exceeded total timeout", timeout=self.client.timeout
                )
...
class DirtyAsyncStreamIterator:
    ...
    async def _read_next_chunk(self):
        ...
        try:
            ...
            if remaining > self._TIMEOUT_THRESHOLD:
                response = await DirtyProtocol.read_message_async(self.client._reader)
            else:
