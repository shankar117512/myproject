...
def _load_cached_py_info(
    ...
    try:
        ...
        py_info_store.remove()
        return None
    if (sys_exe := py_info.system_executable) is not None and not Path(
        sys_exe
    ).exists():
        py_info_store.remove()
        return None
...
def _extract_between_cookies(
    out: str, start_cookie: str, end_cookie: str
...
def _run_subprocess(
    ...
    out, raw_out, out_starts, out_ends = _extract_between_cookies(
        out, start_cookie, end_cookie
    )
    try:
