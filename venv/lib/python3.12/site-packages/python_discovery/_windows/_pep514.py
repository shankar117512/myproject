...
def process_set(
    ...
    try:
        ...
        ) as root_key:  # ty: ignore[unresolved-attribute]
...
def process_company(
    ...
    with winreg.OpenKeyEx(
        root_key, company
    ) as company_key:  # ty: ignore[unresolved-attribute]
...
) -> _RegistrySpec | None:  # noqa: ANN401
    with winreg.OpenKeyEx(
        company_key, tag
    ) as tag_key:  # ty: ignore[unresolved-attribute]
...
) -> tuple[str, str | None] | None:  # noqa: ANN401
    key_path = f"{hive_name}/{company}/{tag}"
    try:
        with winreg.OpenKeyEx(
            company_key, rf"{tag}\InstallPath"
        ) as ip_key, ip_key:  # ty: ignore[unresolved-attribute]
...
def parse_version(
    version_str: Any,
) -> tuple[int | None, int | None, int | None]:  # noqa: ANN401
