...
def get_interpreter(
    ...
    for spec_str in specs:
        if result := _find_interpreter(
            spec_str, try_first_with or (), cache, env, predicate
        ):
            return result
    return None
...
def propose_interpreters(
    ...
    if spec.is_abs and spec.path is not None:
        if exe_raw := _check_exe(
            spec.path, tested_exes
        ):  # pragma: no branch # first exe always new
    ...
    if (
        spec.path is not None and spec.is_abs
    ):  # pragma: no cover # relative spec.path is never abs
        return
...
def get_paths(env: Mapping[str, str]) -> Generator[Path, None, None]:
    ...
    if path is None:
        try:
            ...
            AttributeError,
            ValueError,
        ):  # pragma: no cover # Windows only (no confstr)
            path = os.defpath
    if path:
...
def _read_python_version_file(
    start: str | Path, *, search_parents: bool = True
) -> list[str] | None:
    ...
    while True:
        ...
        if Path(candidate).is_file():
            with Path(candidate).open(encoding="utf-8") as fh:
                if versions := [
                    v for line in fh if (v := line.strip()) and not v.startswith("#")
                ]:
                    return versions
